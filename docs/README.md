...
- A Themes page with a visual region designer, Layers panel, canvas controls, region properties, Program Region editing, static Logo/Image/Text regions, and live Clock regions.
...
11. Add a Clock Region, choose a format, and confirm the designer preview updates every second.
12. Refresh Themes and confirm all regions are restored from JSON.
13. Open Scheduler and assign the custom theme to an active program block.
14. Confirm `GET http://localhost:3000/api/schedule` includes `theme` metadata and the existing `items` array.
15. Start the agent and confirm it writes the enhanced `schedule.json` and caches static region image files.
16. Start the player and confirm media renders in this order: background, images, program, logos, text, clock.
17. Stop the server.
18. Confirm cached playback continues and the clock keeps updating from local player time.
